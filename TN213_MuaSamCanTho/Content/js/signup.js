...
$(document).ready(function () {
    ...
    $("#myform").submit(function () {
        var notMatch = $("#checkPasswordMatch").html() != "Passwords match.";
        if (notMatch) return false;
        var captchaNull = $("#CaptchaInputText").val().trim() == "";
        
        if (captchaNull) {
            $("#checkCaptchaNull").html("Please input captcha!").css("color", "red");
            return false;
        }
        else {
            $("#checkCaptchaNull").html("")
        }
    })
})
